...
function NoticeContent() {
  ...
  const noticesQuery = useListPublicNotices({
    lang,
    ...
    <>
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              >
                ...
                />
                <span
                  className={`absolute top-4 left-4 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold backdrop-blur ${categoryColor(featured.category)}`}
                >
      ...
      <section className="py-20 bg-slate-50">
        <div className="container mx-auto px-6">
          ...
          ) : visible.length === 0 && archive.length === 0 && !featured ? (
            <div className="py-24 text-center text-muted-foreground">
              <p className="text-base">{t("notice.empty") as string}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {(activeCategory || search ? notices : visible).map((item) => (
                <Link
                  key={item.id}

    ...
    <>
      <section className="relative w-full h-[400px] flex items-center">
        <div className="absolute inset-0 z-0">
          <img src={dostacImage("hero-production.png")} alt="" className="w-full h-full object-cover object-center" />
          <div className="absolute inset-0 bg-gradient-to-b from-primary/55 via-primary/45 to-primary/65"></div>
        </div>
        <div className="container relative z-10 mx-auto px-6 text-center text-white">
          <h1 className="font-display text-4xl md:text-5xl font-bold leading-tight mb-4">{t("production.heroTitle") as string}</h1>
        </div>
      </section>

      <section className="py-20 bg-white">
        <div className="container mx-auto px-6 max-w-4xl text-center">
          <p className="text-xl text-muted-foreground leading-relaxed">{t("production.intro") as string}</p>
        </div>
      </section>

      <section className="py-24 bg-muted/30">
        <div className="container mx-auto px-6">
          <h2 className="font-display text-3xl font-bold text-center text-primary mb-16">{t("production.qaHeading") as string}</h2>
              ...
              return (
                <div key={idx} className="relative bg-white p-8 rounded-xl border shadow-sm flex flex-col items-center text-center">
                  ...
                  <div className="font-display font-bold text-lg text-primary mb-3">
                    {t("production.stepLabel") as string} {idx + 1}: <br />{step.title}
                  </div>
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="font-display text-3xl font-bold text-primary mb-4">{t("production.certsHeading") as string}</h2>
            <p className="text-muted-foreground text-lg">{t("production.certsSub") as string}</p>
          </div>
      ...
      <section className="py-20 bg-primary text-white text-center">
        <div className="container mx-auto px-6">
          ...
          <Link href="/contact" className="inline-flex h-12 items-center justify-center rounded-sm bg-accent px-8 text-base font-medium text-white shadow hover:bg-accent/90 transition-colors">
